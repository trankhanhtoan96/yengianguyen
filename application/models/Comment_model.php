...
{
	private $_tkt_table_name = 'comment';
	private $_tkt_fields;
	...
	public function tkt_get_list($limit=0,$offset=0,$order='DESC',$fieldoder='',$blog_id=0)
	{
		if($limit!=0)
		{
	...
	function tkt_update_status($blog_id)
	{
		$data = array(
			'comment_check' => '1'
			);
		$this->db->where('comment_blog_id',$blog_id);
		if($this->db->update($this->_tkt_table_name,$data)) return TRUE;
		return FALSE;
	}

	function get_new()
	{
		$this->db->where('comment_check',0);
		$this->db->group_by('comment_blog_id');
		$this->db->select('comment_blog_id');
		$result = $this->db->get($this->_tkt_table_name);
		return $result->result_array();
	}
}