...

{
...

	{
...

		{
			...
			if($this->tkt_upload->tkt_upload('slide_image_url','image','images/slides/'))

			{

				$data_insert['slide_image_url'] = $this->tkt_upload->tkt_get_file_path();
...

	{
...

		{
			...
			if($this->order_model->tkt_delete($this->input->post('table_records')))
		...
		$data['_content'] = 'admin/order';

		$data['_varibles']['orders'] = $this->order_model->tkt_get_list();

		$this->load->view('admin/index',$data);

	}

	function hoanthanh($order_id)
	{
		$data = array('order_checked'=>1);
